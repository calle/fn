var snitch = function($) {
	...
	function updateLists(message) {
		...
		var i, n, currUser, prevData, 
				newItems 		= [],
				hasPrevData = false;
		
		var currentGoodList = currentLists.good;
		var currentBadList = currentLists.bad;
		currentLists.good = [];
		currentLists.bad = [];
		
		var from = '#ECF3FE', to, id;
		for (i = 0, n = goodList.length; i < n; i++) {
			currUser = goodList[i];
			prevData = currentGoodList[currUser.name];
			id = 'goodlist-item-' + i + new Date().getTime(); 
			
			if (prevData) {
				console.log("Giving pos " + prevData.pos + "("+currUser.name + ") color " + ((i < prevData.pos) ? 'blue' : 'red'));
				$('#' + prevData.elementId).animate({
					backgroundColor: (i < prevData.pos) ? '#77A9F8' : '#F88677'
				}, 1000);
				hasPrevData = true;
			} 
			newItems.push(_createListItem(currUser, id));
			
			currentLists.good[currUser.name] = {
				pos: i,
				points: currUser.points,
				elementId: id
			};
		}
		var newBolshevikList = ['<ul class="feed-list hover-menu nav-list" id="bolsheviks-list">',
			newItems.join(''),
		'</ul>'].join('');
		
		newItems = [];
		for (i = 0, n = badList.length; i < n; i++) {
			currUser = badList[i];
			prevData = currentBadList[currUser.name];
			id = 'badlist-item-' + i + new Date().getTime(); 
			...
			currentLists.bad[currUser.name] = {
				pos: i,
				points: currUser.points,
				elementId: id
			};
		...
		var newMenshevikList = ['<ul class="feed-list hover-menu nav-list" id="mensheviks-list">',
			newItems.join(''),
		'</ul>'].join('');
		
		var updateFunction = function() {
			$('#bolsheviks-list').replaceWith(newBolshevikList);
			$('#mensheviks-list').replaceWith(newMenshevikList);
		};
	...
	return {
		init: function() {
			// Attach event handlers to Socket.IO events
			if (developerMode) {
